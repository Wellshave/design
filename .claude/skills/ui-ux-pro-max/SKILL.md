...
The search script lives inside this skill's own directory, not the project directory. The paths below are written relative to the project root; if the working directory is elsewhere, prefix them with the project root so the script resolves:
